...
class TrainValidImageDataset(Dataset):
    ...
    Args:
        ...
        image_size (int): Low resolution image size.
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        if self.mode == "Train":
            image = imgproc.random_rotate(image, angles=[0, 90, 180, 270])
            image = imgproc.random_horizontally_flip(image, p=0.5)
        ...
        tensor = imgproc.image2tensor(image, range_norm=False, half=False)
